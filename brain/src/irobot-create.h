...
# define IROBOT_MAKE_EXCEPTION(NAME, DESCRIPTION)\
  ...
  {\
    explicit NAME(): std::runtime_error(DESCRIPTION)\
    {}\
    explicit NAME(const std::string & what): std::runtime_error(what)\
    {}\
  }
...
public:
  ...
  void sendPwmLowSideDriversCommand (unsigned char lsd1, unsigned char lsd2, unsigned char lsd3)
  throw(CommandNotAvailable, InvalidArgument);
